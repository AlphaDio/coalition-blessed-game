Add a set of Emergency Laws that apply powerful timed modifiers but at the cost of supplies and some goods during that duration # IN REVIEW

Make the laws enacted give a "color" to the Coalition, pretty much adopting a Spiritual law should give a Spiritual coloration to the Coalition # DONE

# DONE
- Biomass -> Increases population
- Rare Gases -> Increases Research Speed
- Ancient Relics -> Increases Fervor Contribution to Army Power
- Solid Ice -> Supply efficiency
- Super Alloys -> Construction
- Sentient Cores -> Law Progress
# DONE

Show relations in detailed Empire view # DONE

Make the a Market side-wide that lists a resource and all its market orders # DONE

Add a modifier to increase the Coalition construction additively and multiplicatively # DONE

Extract improvements into modules # DONE

Do a pass of the Economy Laws, Military Laws, and Stability Laws, Ethics Laws and Diplomacy Laws # DONE

Do a pass of all T1 Improvements # DONE

Add a modifier that can be used as a lever to commodities-to-supplies conversion # REVIEW

Make sure the Emergency Laws work and are temporary # REVIEW

Show the Empire the Improvement belongs to in the Work view # DONE

Show the Improvements the Empire has in the Empire details view # DONE

Make the resources.yaml follow the ds.yaml format

Extract log making
